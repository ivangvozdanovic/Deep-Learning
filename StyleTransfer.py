...
def neural_style_transfer(config):
    ...
    dump_path = config['style_output_dir'] #os.path.join(config['output_img_dir'], out_dir_name)
    #full_output_path = ''
    ...
    num_of_iterations = config['iter_num']
    num_iteration = num_of_iterations[ config['optimizer'] ]
    ...
    if config['optimizer'] == 'adam':
        ...
        for cnt in range( num_iteration ):
            ...
            with torch.no_grad():
                ...
                full_output_path = utils.save_and_maybe_display(optimizing_img, dump_path, config, cnt, num_iteration, should_display=False)
    elif config['optimizer'] == 'lbfgs':
        print("Using LBFGS")
        # line_search_fn does not seem to have significant impact on result
        optimizer = LBFGS((optimizing_img,), max_iter=num_iteration, line_search_fn='strong_wolfe')
        cnt = 0
    ...
    return dump_path#, full_output_path